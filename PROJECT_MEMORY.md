...
- 产品：名称已确认采用 Luma（原玄衡）；四柱八字、塔罗占问、紫微斗数、合缘已采用用户选定的“日光之间”视觉系统。
- 阶段：V1 八字、V2 塔罗、V3.2 紫微与 V4 合缘均为本地 RC；紫微功能闭环已完成但排盘仍待 30 盘校准。
- 当前工作：日光之间正式前端重构已实现，等待用户视觉验收；V1/V2/V3/V4 联合验收及紫微校准继续保留。不部署云端。
- 尚未开始：正式上线、云端持久化、账户/跨设备同步、上线后 D 类验收。
- 最新功能基线：当前 `main` 分支最新提交；具体提交号从 Git 历史读取，不在正文循环追写。
- 本地入口：双击 `打开Luma.command`（旧入口继续兼容），并保持服务终端运行；直接打开 `index.html` 不能使用后端功能。
...
1. 验收日光之间正式首页与内部页面；重点检查卡片导航、报告阅读、历史侧栏、出生档案弹窗与移动端。
